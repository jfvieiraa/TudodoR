...
points(solo2$ds,solo2$ptotal,pch=2, col="blue") 
points(solo3$ds,solo3$ptotal,pch="%")
```
...
lines(solo2$ds,solo2$ptotal, lty=2) #linha interrompida
```
...
Você pode definir facilmente o tamanho e o estilo da fonte, mas a familia de fontes é um pouco mais complicada.

**Fonte**	Número inteiro que especifica a fonte a ser usada para texto. 
1 = simples, 2 = negrito, 3 = itálico, 4 = negrito itálico, 5 = símbolo
...
plot(solo1$ds,solo1$ptotal,
     main="Fatorial", 
        ...
        cex.lab=1.5,                        #ampliação dos rótulos x e y em relação ao cex
        cex.main=1.5,                       #ampliação de títulos em relação ao cex
        font.axis=3,                        #fonte para anotação de eixo
        font.main=4,                        #fonte para títulos
        font.lab=4,                         #fonte para rótulos
        text(1.5,0.5,family="A", "Engenharia Agrícola",cex =1,pos =2))
