...
Podemos verificar o cabe?ario do conjunto de dados
```{r}
head(roo)
```
...
```{r}
trmm <- read.csv2(file ="https://www.dropbox.com/s/v16z0cq7i45q5u7/roo_trmm.csv?dl=1", sep = "," )
head(trmm)
```
...
Com a fun??o gather iremos unir todos valores  de chuva mensal em uma coluna separado pelo meses
...
##1.3 Visualizar dados temporais no R

*Vamos baixar os dados clim?ticos de Rondon?polis-MT*
```{r}
roo <- read.csv2("https://www.dropbox.com/s/bso8sh05meu5zxh/roo.csv?dl=1")
head(roo)
tail(roo)
```
...
## 1.4 Trabalhando com datas no R

O Pacote Lubridate fornece ferramentas que facilitam a an?lise e manipula??o de datas. Essas ferramentas s?o agrupadas abaixo por um prop?sito comum. Mais informa??es sobre cada fun??o podem ser encontradas em sua documenta??o de ajuda.
...
YYYY-MM-DD: corresponde ao ano, m?s e dia.
...
O objeto *x* ? um caracter, precisamos transformar para data com a fun?ao **as.Date()**
...
convertendo datas n?o padronizadas para padr?o
